...
class SmartcarOAuth2FlowHandler(AbstractOAuth2FlowHandler, domain=DOMAIN):  # type: ignore[call-arg]
    ...
    async def async_step_scopes(
        ...
        _LOGGER.debug(
            f"Handler {self.flow_id}: Starting step_scopes, input: {user_input}"
        )
        errors: dict[str, str] = {}
        ...
        def default(scope):
            if user_input is None:
                ...
                return user_input.get(scope, False)
    ...
    async def async_oauth_create_entry(self, data: dict) -> FlowResult:
        ...
        except EmptyVehicleListError as err:
            _LOGGER.error(f"No vehicles returned: {err}")
            return self.async_abort(reason="no_vehicles")
        except MissingVINError as err:
            _LOGGER.error(f"Missing vehicle VIN: {err}")
            return self.async_abort(reason="unknown")
        except InvalidAuthError as err:
            _LOGGER.error(f"Failed to authenticate: {err}")
            return self.async_abort(reason="invalid_access_token")
        except (ClientConnectorError, ClientError) as err:
            _LOGGER.error(f"Failed to fetch vechicles: {err}")
