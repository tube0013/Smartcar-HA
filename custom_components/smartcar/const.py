...
class Scope(StrEnum):
    ...
    READ_BATTERY = auto()
    READ_CHARGE = auto()
    READ_CHARGE_EVENTS = auto()
    READ_CHARGE_LOCATIONS = auto()
    READ_CHARGE_RECORDS = auto()
    READ_CLIMATE = auto()
    READ_COMPASS = auto()
    READ_DIAGNOSTICS = auto()
    READ_ENGINE_OIL = auto()
    READ_EXTENDED_VEHICLE_INFO = auto()
    READ_FUEL = auto()
    READ_LOCATION = auto()
    READ_ODOMETER = auto()
    READ_SECURITY = auto()
    READ_SERVICE_HISTORY = auto()
    READ_SPEEDOMETER = auto()
    READ_THERMOMETER = auto()
    READ_TIRES = auto()
    READ_USER_PROFILE = auto()
    CONTROL_CHARGE = auto()
    CONTROL_CLIMATE = auto()
    CONTROL_NAVIGATION = auto()
    CONTROL_PIN = auto()
